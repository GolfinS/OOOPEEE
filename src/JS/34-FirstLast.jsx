...
const FirstLast = () => {
    ...
    const checkFirstLast6 = () => {
        const Data = arraySize.split(',').map(Number);
        if (Data.length >= 1) {
            const first = Data[0];
            const last = Data[Data.length - 1];
            setResult(first === 6 || last === 6);
        } else {
            setResult(false);
        }
    };
         ...
         <div className="min-h-92.5 bg-rose-200 flex items-center justify-center">
                <div className="relative w-[700px]">
                    <div className="relative px-20 py-12 bg-gradient-to-r from-rose-700 to-pink-600 rounded-[10px] shadow-[-11px_14px_49px_13px_#26261BA8]">
                        <div className="mb-8 text-center text-5xl font-extrabold text-white">FirstLast6</div>
            <input
                type="text"
                value={arraySize}
    ...
    <div className='flex '>
            ...
            onClick={checkFirstLast6}>Check</button>
            <button 
